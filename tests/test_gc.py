...
class TestGc(unittest.TestCase):
    def test_delete_packs_before(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            old_pack = {"schema_version": "1", "created_at": (now - timedelta(days=10)).isoformat().replace("+00:00", "Z"), "customer_id": "c1"}
            new_pack = {"schema_version": "1", "created_at": (now - timedelta(days=1)).isoformat().replace("+00:00", "Z"), "customer_id": "c1"}
            ...
            deleted = store.delete_packs_before(cutoff=now - timedelta(days=5), customer_id="c1", dry_run=False)

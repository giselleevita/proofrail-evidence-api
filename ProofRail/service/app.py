...
class CreateKeyRequest(BaseModel):
    ...
    scopes: str = Field(default="write:screen,read:evidence", max_length=512)
...
def create_app(*, ingest_func=ingest_sources) -> FastAPI:
    ...
    @app.get("/v1/evidence-packs/{evidence_pack_id}/signature")
    ...
    @app.post("/v1/evidence-packs/verify")
    ...
    def admin_create_key(req: CreateKeyRequest) -> CreateKeyResponse:
        ...
        db.create_api_key(api_key_id, req.customer_id, hash_api_key(api_key), req.scopes, utc_now_iso())
    ...
    def admin_revoke_key(api_key_id: str) -> dict[str, str]:
        db.revoke_api_key(api_key_id, utc_now_iso())
        return {"status": "revoked"}

    @app.post("/v1/admin/keys/rotate", response_model=CreateKeyResponse, dependencies=[Depends(require_admin)])
    def admin_rotate_key(customer_id: str, scopes: str = "write:screen,read:evidence") -> CreateKeyResponse:
        # Rotation creates a new key; caller can revoke old ones separately.
        db.ensure_customer(customer_id, utc_now_iso())
        api_key = generate_api_key()
        api_key_id = sha256_hex(api_key.encode("utf-8"))[:16]
        db.create_api_key(api_key_id, customer_id, hash_api_key(api_key), scopes, utc_now_iso())
        return CreateKeyResponse(customer_id=customer_id, api_key_id=api_key_id, api_key=api_key, scopes=scopes)
    ...
    @app.post("/v1/sanctions/screen", response_model=ScreenResponse)
    ...
    @app.get("/v1/evidence-packs/{evidence_pack_id}")
